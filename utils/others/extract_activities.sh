...
print_folder_recurse() {
    ...
    do
		...
		nvm_read=$(grep "CacheLLC:NVMways:reads"  $DIR/results_1.out | awk -F "\t" '{print $2}' )
		nvm_write=$(grep "CacheLLC:NVMways:writes"  $DIR/results_1.out | awk -F "\t" '{print $2}' )
		sram_read=$(grep "CacheLLC:SRAMways:reads"  $DIR/results_1.out | awk -F "\t" '{print $2}' )
		sram_write=$(grep "CacheLLC:SRAMways:writes"  $DIR/results_1.out | awk -F "\t" '{print $2}' )
		dram_access=$(grep "MainMem"  $DIR/results_1.out | awk -F "\t" '{print $2}' | awk '{sum += $1} END {print sum}')
		
		echo -e $DIR"\t"$sram_read" "$sram_write" "$nvm_read" "$nvm_write" "$dram_access >> $2
