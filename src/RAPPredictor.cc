...
RAPPredictor::updatePolicy(uint64_t set, uint64_t index, bool inNVM, Access element, bool isWBrequest = false)
{
	...
	DPRINTF("RAPPredictor::updatePolicy set %ld , index %ld\n" , set, index);
...
void
RAPPredictor::checkLazyMigration(RAPEntry* rap_current , CacheEntry* current ,uint64_t set,bool inNVM, uint64_t index)
{
	if(rap_current->des == ALLOCATE_IN_NVM && inNVM == false)
	{
		//Trigger Migration
		int id_assoc = evictPolicy(set, true);//Select LRU candidate from NVM cache
		DPRINTF("RAPPredictor:: Migration Triggered from SRAM, index %ld, id_assoc %d\n" , index, id_assoc);
...
void
RAPPredictor::insertionPolicy(uint64_t set, uint64_t index, bool inNVM, Access element)
{
	CacheEntry* current = NULL;
	if(inNVM)
		current = m_tableNVM[set][index];
...
void
RAPPredictor::updateWindow(RAPEntry* rap_current)
{
	if(rap_current->des == BYPASS_CACHE) 
	//if(false)
	{
...
RD_TYPE
RAPPredictor::evaluateRd(vector<int> reuse_distances)
{
